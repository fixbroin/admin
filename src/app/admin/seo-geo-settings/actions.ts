...
export const getAllSeoData = cache(async (): Promise<Record<string, PageSeoContent>> => {
    return await unstable_cache(
        async () => {
            try {
                const slugs = Object.keys(defaultContent);
                const allSeo: Record<string, PageSeoContent> = {};
                
                for (const slug of slugs) {
                    allSeo[slug] = await getSeoData(slug);
                }
        ...
        ['all-seo-data'],
        { tags: ['settings', 'seo-data'], revalidate: 86400 }
    )();
});
